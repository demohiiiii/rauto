...
然后访问 `http://127.0.0.1:3000` 并输入 Web 密码。第一次启动
`rauto web` 时，rauto 会生成随机密码，在终端中显示一次，并保存到
`~/.rauto/config.toml`：
...
在 Unix 系统上，该配置文件只允许当前用户访问。需要修改密码时，请先停止
Web 服务，再修改 `web.password`。浏览器会话只保存在内存中，rauto 重启后需要
重新登录。
...
- 在页面中管理 `RAUTO_HOME/backups` 下的备份归档；Web API 不接受任意主机输出路径或恢复路径。
...
`targets` 中的每一项都必须是已保存连接的名称。不再支持内联目标对象或在
任务内覆盖连接参数；请改为使用已保存连接本身的属性和变量。
...
`core` 和 `access` 设备组必须已存在于 rauto 中。可在 Web 工作台或通过
`rauto inventory group` 管理组内的已保存连接。
...
- `targets` 必须引用已保存连接名。
- `target_groups` 选择 rauto 中持久化的设备组，`target_tags` 选择已保存连接的标签；多个组和标签使用并集语义。
- 设备组和标签匹配结果按已保存连接名去重。
- `tx_workflow` job 只能选择一种来源：
  - 内联 `workflow`
  - 已保存的 `workflow_template_name`，可选 `workflow_vars`
...
saved connection 就是 inventory 目标的来源（包含 `enabled`、`labels` 和连接 `vars` 等元数据），
Inventory CLI 只负责成员关系型设备组的管理。
...
- `~/.rauto/backups`（备份归档）
- `~/.rauto/uploads`（供 Web/API SFTP 上传的暂存文件）
- `~/.rauto/keys`（Web/API 凭证允许使用的私钥文件）
