...
} from "../src/domains/profiles/application/profilePanelWorkspaces.js";
import {
  builtinProfileReadonlyDisplay,
  hookOperationEditorDisplay,
} from "../src/domains/profiles/presentation/profileEditorPresentation.js";

type LoggedValue = string | number | boolean | object;
type LoggedCall = [label: string, ...values: LoggedValue[]];

test("profile list row handlers use the latest row context", () => {
  const calls: LoggedCall[] = [];
  const workspace = createProfileListRowEditorWorkspace({
    onPatternChange: (rowIndex, patternIndex, value) =>
      calls.push(["pattern", rowIndex, patternIndex, String(value)]),
...
test("profile inputs normalize DOM events before updating list state", () => {
  ...
  const workspace = createProfileListRowEditorWorkspace({
    onPatternChange: (rowIndex, patternIndex, value) =>
      calls.push(["pattern", rowIndex, patternIndex, String(value)]),
    onProfileListRowChange: (...args) => calls.push(["row", ...args]),
    onSimpleValueChange: (rowIndex, value) =>
      calls.push(["simple", rowIndex, String(value)]),
  });
  const inputEvent = (value: string) => ({ currentTarget: { value } });
