...
import type { JsonTemplateActionContext } from "../src/domains/transactions/index.js";

interface TextFile {
  text(): Promise<string>;
}

interface CurrentActionContext extends JsonTemplateActionContext {
  isCurrent(): boolean;
}
...
test("context-before-connect synchronizes once and value changes do not reconnect", () => {
  ...
  const syncCalls: Parameters<typeof setTxJsonEditorRawText>[] = [];
  const onInput = () => {};
  const workspace = createTxJsonEditorWorkspace({
    connectHost: () => {
      connectionCount += 1;
...
test("mounted workflow import owns its notification and publishes success once", async () => {
  ...
  assert.equal(
    requireCurrentActionContext(importActionContext).isCurrent(),
    true,
  );
  assert.equal(
    get(transactionOutputState(TX_OUTPUT.txWorkflowPlan)).tone,
...
test("mounted workflow template create owns its normalization notification", async () => {
  ...
  let createActionContext: CurrentActionContext | null = null;
  let selectedName = "";
  const statuses: Array<[string, string, string, string]> = [];
  const library = createJsonTemplateLibrary({
    ...
    setNamedStatus: (
      ...
      fallback: string,
      name: string,
    ) => statuses.push([output, action, fallback, name]),
  ...
  const inputWorkspace = createTxWorkflowInputPanelWorkspace({
    onCreateJsonTemplateDraft(actionContext: CurrentActionContext) {
  ...
  assert.equal(modelPublications, 1);
  assert.equal(
    requireCurrentActionContext(createActionContext).isCurrent(),
    true,
  );
...
test("orchestration draft reset stays current until a real editor input", async () => {
  ...
  let createActionContext: CurrentActionContext | null = null;
  const editorWorkspace = createOrchestrationEditorPanelWorkspace({
    onCreateDraft(actionContext: CurrentActionContext) {
      createActionContext = actionContext;
...
test("orchestration draft refresh clears changed diagnostics without republishing the same model", async () => {
  clearTxJsonEditorsHost();
  let editorWorkspace: ReturnType<
    typeof createOrchestrationEditorPanelWorkspace
  >;
