...
test("transaction commands normalize and serialize multiline modes", () => {
  ...
  assert.equal(model.steps[0].run.command.multilineMode, "split_lines");
  const stepRollback = model.steps[0].rollback;
  assert.ok(stepRollback);
  assert.equal(stepRollback.command.multilineMode, "whole");
  ...
  assert.ok(wholeResource);
  assert.equal(wholeResource.rollback.command.multilineMode, "whole");
  assert.equal(json.steps[0].run.multiline_mode, "split_lines");
...
test("keeps the previous form model and exposes parse detail for invalid JSON", () => {
  ...
  const errorDetail = state.formErrorDetail;
  assert.ok(errorDetail);
  assert.equal(errorDetail.message, state.formError);
  assert.equal(errorDetail.line, 3);
  assert.equal(errorDetail.column, 1);
});

test("returns validation errors in stable execution order", () => {
  const invalid = {
    ...structuredClone(completeTxBlock),
    steps: [
      {
        run: {
          kind: "flow",
