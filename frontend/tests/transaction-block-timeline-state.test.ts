...
interface TimelineStepModel extends TxStepFormModel {
  extra: { nested: { value: string } };
}

interface TimelineBlockModel extends TxBlockFormModel {
  extra: JsonObject;
  steps: TimelineStepModel[];
}

function command(value = ""): TxCommandModel {
  return {
    command: value,
    dynParams: {},
    extra: {},
    ...
    hasTimeout: false,
    interaction: { extra: {}, hasPrompts: false, prompts: [] },
    mode: "Enable",
...
function flow(): TxFlowModel {
  return {
    extra: {},
    hasMaxSteps: false,
    hasStopOnError: false,
    maxSteps: null,
    steps: [command("show version"), command("show clock")],
    stopOnError: true,
  };
}

function operation(kind: TxOperationKind, value = ""): TxOperationModel {
  return { command: command(value), flow: flow(), kind };
}

function step(
  kind: TxOperationKind,
  value = "",
  rollback: TxOperationModel | null = null,
): TimelineStepModel {
  return {
    extra: { nested: { value } },
...
function model(steps: TimelineStepModel[] = []): TimelineBlockModel {
  return {
    extra: {},
    name: "deploy edge",
    failFast: true,
...
test("step rollback switch creates and clears the compensating operation", () => {
  ...
  const enabledRollback = enabled.steps[0].rollback;
  assert.ok(enabledRollback);
  assert.equal(enabledRollback.kind, "command");
...
test("duplicates a deeply independent step immediately after its source", () => {
  ...
  const duplicateSteps = duplicate.steps as TimelineStepModel[];
  duplicateSteps[1].run.command.command = "changed";
  duplicateSteps[1].extra.nested.value = "changed";
  assert.equal(duplicateSteps[0].run.command.command, "one");
  assert.equal(duplicateSteps[0].extra.nested.value, "one");
...
test("moves steps immutably and leaves the source model unchanged", () => {
  ...
  (moved.steps as TimelineStepModel[])[2].extra.nested.value = "changed";
...
test("editor bindings expose semantic duplicate and move actions", () => {
  const source = model([step("command", "one"), step("command", "two")]);
  const changes: TxBlockFormModel[] = [];
