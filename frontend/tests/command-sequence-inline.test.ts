...
test("inline command sequence steps move immutably", () => {
...
test("inline command sequence steps duplicate deeply", () => {
  const source = flowOperation();
...
test("inline sequence settings preserve the rneter command flow shape", () => {
  const source = flowOperation();
...
test("inline sequence bindings notify once and keep the source operation unchanged", () => {
  const source = flowOperation();
