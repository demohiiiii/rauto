...
interface ExternalActionFile {
  name: string;
}

type ExternalActionResult = string | Promise<string>;

interface ExternalDependencies extends Record<string, unknown> {
  onCreateDirectDraft(context?: ExternalActionContext): ExternalActionResult;
  onCreateJsonTemplateDraft(
    context?: ExternalActionContext,
  ): ExternalActionResult;
  onEditorInput(jsonText: string): void;
  onImportFile(
    file?: ExternalActionFile,
    context?: ExternalActionContext,
  ): ExternalActionResult;
  onLoadJsonTemplate(
    templateName?: string,
...
const formModelToJsonText = (model: TestFormModel): string =>
  JSON.stringify(model, null, 2);

function inputFormStateFromJsonText(
  jsonText: string,
  currentModel: TestFormModel,
): TestParsedFormState {
  try {
    return {
      formError: "",
      formErrorDetail: null,
      formModel: JSON.parse(jsonText) as TestFormModel,
    };
  } catch (error) {
...
function createExternalActionHarness({
  notifyInput = true,
}: { notifyInput?: boolean } = {}) {
  let actionWorkspace: TestActionWorkspace;
  let externalJsonText = formModelToJsonText(buildDefaultFormModel());
  let parserCalls = 0;
  const dependencyObservations: Array<{
    formModel: TestFormModel;
    parserCalls: number;
  }> = [];
  const snapshots: Array<
    TransactionEditorSessionState<TestFormModel, JsonErrorDetail>
  > = [];
  ...
  const dependencies: ExternalDependencies = {
    ...
    onEditorInput: (jsonText) => {
      upstreamTexts.push(jsonText);
    },
  ...
  return {
    ...
    setExternalJsonText: (jsonText: string) => {
...
function createDeferredPromise<T = string>(): Deferred<T> {
...
function createTemplateCreateRaceHarness() {
  ...
  const library = createJsonTemplateLibrary({
    ...
    normalizeEditorKey: (editorKey: string) => editorKey,
    promptForResourceName: () => promptNames.shift() || "",
    setSelectedName: (_kind: string, name: string) => {
      selectedName = name;
    },
    txEditor: TX_EDITOR,
    txTemplateKind: TX_TEMPLATE_KIND,
    updateOptions: (updatedKind: string, options: JsonTemplateSelectState) =>
  ...
  return {
    ...
    listRequests,
    latestOptions: () =>
      [...optionUpdates].reverse().find((update) => update.kind === kind)
        ?.options,
    nextActionContext() {
...
test("form changes update JSON and publish by default", () => {
  const published: Array<[TestFormModel, string]> = [];
  const session = createSession((model, jsonText) => {
    published.push([model, jsonText]);
  });
    ...
    await testContext.test(editorCase.name, async (editorTest) => {
      for (const [jsonText, errorKeyName] of invalidRoots) {
        await editorTest.test(JSON.stringify(jsonText), () => {
          ...
          const errorKey =
            errorKeyName === "requiredKey"
              ? editorCase.requiredKey
              : editorCase.shapeKey;
          assert.equal(session.replaceJsonText(jsonText), false);
          assert.equal(get(session.jsonTextStateStore), jsonText);
          assert.equal(session.currentFormModel(), priorModel);
          assert.equal(get(session.lastValidJsonStateStore), priorJsonText);
          assert.equal(get(session.formErrorStateStore), t(errorKey));
          assert.deepEqual(get(session.formErrorDetailStateStore), {
            message: t(errorKey),
            line: null,
            column: null,
...
test("workspace JSON input returns parsed state and updates canonical stores", () => {
  let parserCalls = 0;
  const workspace = createTxInputPanelWorkspace<TestFormModel, JsonErrorDetail>(
    {
      buildDefaultFormModel,
      formModelToJsonText,
...
test("workspace form changes save once with the requested notify flag", () => {
  const saved: Array<[TestFormModel, { notify?: boolean } | undefined]> = [];
  const workspace = createTxInputPanelWorkspace<TestFormModel, JsonErrorDetail>(
    {
      buildDefaultFormModel,
      formModelToJsonText,
...
test("notified external actions parse immediately and do not refresh twice", async (t) => {
  const actionCases = [
    {
      name: "createJsonDraft",
      run: (workspace: TestActionWorkspace) => workspace.createJsonDraft(),
    },
    {
      name: "createTemplateDraft",
      run: (workspace: TestActionWorkspace) => workspace.createTemplateDraft(),
    },
    {
      name: "createDirectDraft",
      run: (workspace: TestActionWorkspace) => workspace.createDirectDraft(),
    },
    {
      name: "importFile",
      run: (workspace: TestActionWorkspace) =>
        workspace.importFile({ name: "draft.json" }),
    },
    {
      name: "loadJsonTemplate",
      run: (workspace: TestActionWorkspace) =>
        workspace.loadJsonTemplate("saved-template"),
    },
  ];
  ...
  for (const actionCase of actionCases) {
    await t.test(actionCase.name, async () => {
      ...
      assert.equal(
        await actionCase.run(harness.actionWorkspace),
        "external-result",
      );
      harness.unsubscribe();
...
test("manual block form changes invalidate a pending template load", async () => {
  ...
  assert.equal(loadActionContexts.at(-1)?.isCurrent(), false);
