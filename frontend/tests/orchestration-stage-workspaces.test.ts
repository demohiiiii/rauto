...
import type { OrchestrationPlanFormModel } from "../src/domains/orchestration/index.js";

function planModel({
  secondStage = false,
}: { secondStage?: boolean } = {}): OrchestrationPlanFormModel {
  const stages = [
    {
      name: "prepare",
      strategy: "serial",
...
test("stage panel workspace adds and removes stages with the latest context", () => {
  const firstChanges: OrchestrationPlanFormModel[] = [];
  const secondChanges: OrchestrationPlanFormModel[] = [];
  const workspace = createOrchestrationStagesPanelWorkspace({
    model: planModel(),
    onChange: (model) => {
      firstChanges.push(model);
    },
  });
...
test("stage and job editor workspaces patch the selected rows", () => {
  const stageChanges: OrchestrationPlanFormModel[] = [];
  ...
  stageWorkspace.setStageContext({
    model: currentModel,
    onChange: (model) => {
      stageChanges.push(model);
    },
    stageRow: orchestrationVisualEditorDisplay(currentModel).stageRows[1],
  });
...
test("plan settings workspace patches root fields", () => {
  const changes: OrchestrationPlanFormModel[] = [];
...
test("target workspaces replace lists and refresh their callbacks", () => {
  const modelChanges: OrchestrationPlanFormModel[] = [];
  ...
  const firstCalls: Array<[string, readonly string[]]> = [];
  const secondCalls: Array<[string, readonly string[]]> = [];
  const editorWorkspace = createOrchestrationJobTargetsEditorWorkspace({
    onReplaceStringList: (...args) => {
