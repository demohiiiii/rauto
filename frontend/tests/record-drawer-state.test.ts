...
import type { RecordDrawerPreferences } from "../src/domains/overlays/index.js";
import { createReplayPageWorkspace } from "../src/domains/replay/index.js";
...
test("record drawer content handlers accept bare values from Plain* field bindings", () => {
  ...
  const workspace = createRecordDrawerContentWorkspace({
    ...
    onFailedOnlyChange: (value) => {
      calls.push(["failed", value]);
    },
    onRawInput: (value) => {
      calls.push(["raw", value]);
    },
    onRecordLevelChange: (value) => {
...
test("record drawer preferences load once and setters save normalized snapshots", () => {
  let loadCount = 0;
  const savedPreferences: RecordDrawerPreferences[] = [];
