...
function renderOrchestrationJsonTreeNode(value, label = "", depth = 0) {
  ...
  const children = entries.length
    ? entries
        .map(([key, item]) =>
          renderOrchestrationJsonTreeNode(item, key, depth + 1),
        )
        .join("")
    : `<div class="rounded-md border border-dashed border-slate-300 bg-slate-50 px-2 py-1 text-xs text-slate-500">{}</div>`;
  return `
...
function orchestrationOperationText(operation) {
  ...
  if (operation.kind === "flow" && Array.isArray(operation.steps)) {
    const first = safeString(
      operation.steps[0] && operation.steps[0].command,
    ).trim();
...
function collectTxWorkflowCommandPreview(workflow) {
  ...
  blocks.forEach((block, blockIdx) => {
    ...
    items.push(...collectTxBlockCommandPreview(block, `block[${blockIdx}]`));
  });
  return items;
}
...
function orchestrationActionCommandPreviewItems(action) {
  ...
  if (action.kind === "tx_block") {
    ...
    if (
      ...
      const inlineBlock = parseJsonObjectSafely(
  ...
  if (action.kind === "tx_workflow") {
    ...
    if (
      ...
      const inlineWorkflow = parseJsonObjectSafely(
    ...
    if (
      typeof action.workflow_file === "string" &&
      action.workflow_file.trim()
    ) {
      return [
...
function renderOrchestrationActionCommandPreview(action) {
  ...
  const rows = trimmed
    .map(
      (line) =>
        `<div class="rounded-md border border-cyan-200 bg-cyan-50 px-2 py-1 font-mono text-xs text-cyan-900 break-all">${escapeHtml(
          line,
        )}</div>`,
    )
    .join("");
...
function renderOrchestrationPreviewHtml(plan, inventory) {
  ...
  const summary = `
    <div class="grid gap-2 md:grid-cols-4">
      ...
      ${renderTxWorkflowPreviewMeta(t("orchestrationVisualGroups"), groupCount)}
    </div>
  `;
  const stageCards = stages
    .map((stage, idx) => {
      ...
      const jobCards = jobs.length
        ? jobs
            .map((job, jobIdx) => {
              const jobLabels = resolveOrchestrationJobTargetsPreview(
                job,
                inventory,
              );
              ...
              return `
                <section class="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                  ...
                  ${
                    Array.isArray(job && job.target_groups) &&
                  ...
                  ${
                    Array.isArray(job && job.target_tags) &&
    ...
    .join("");
  return `<div class="grid gap-3">${summary}${
    stageCards ||
  ...
  if (target.tx_result != null) {
    payloadCards.push(
      renderOrchestrationJsonSection(
        t("orchestrationPayloadTxResult"),
        target.tx_result,
      ),
    );
  }
...
export function installRenderOrchestrationRuntime() {
  Object.assign(window, {
    inventoryGroupTargets,
    orchestrationStageStrategyLabel,
    orchestrationActionSummary,
    orchestrationStatusLabel,
    orchestrationStatusBadge,
    orchestrationJsonText,
    orchestrationJsonPrimitiveText,
    renderOrchestrationJsonTreeNode,
    renderOrchestrationJsonSection,
    parseJsonObjectSafely,
    orchestrationOperationText,
    collectTxBlockCommandPreview,
    parseInlineFlowTemplateCommands,
    collectTxWorkflowCommandPreview,
    orchestrationActionCommandPreviewItems,
    renderOrchestrationActionCommandPreview,
    resolveOrchestrationJobTargetsPreview,
    resolveOrchestrationStageTargetsPreview,
    renderOrchestrationPreviewHtml,
    renderOrchestrationStageDetail,
