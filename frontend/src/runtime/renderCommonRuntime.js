...
function parseModeValidationMessage(message) {
  if (typeof window.parseModeValidationMessage === "function") {
    return window.parseModeValidationMessage(message);
  }
  return null;
}

function defaultFlowVarDraft(field) {
  return callRuntimeFunction("defaultFlowVarDraft", () => "", field);
}

function flowVarTypeLabel(kind) {
  return callRuntimeFunction(
    "flowVarTypeLabel",
    () => safeString(kind || "string"),
    kind,
  );
}
...
function getFlowRunVarsSchema() {
  return callRuntimeFunction(
    ...
    () => [],
    runtimeValue("lastFlowRunTemplateDetail"),
  );
}

function getLastFlowRunTemplateDetail() {
  return runtimeValue("lastFlowRunTemplateDetail");
}

function setLastFlowRunTemplateDetail(detail) {
  setRuntimeValue("lastFlowRunTemplateDetail", detail);
}
...
function renderFlowTemplateVarField(field, draftValue, options = {}) {
  ...
  const placeholderAttr = field.placeholder
    ...
    : "";
  const idPrefix =
    safeString(options.idPrefix || "flow-var").trim() || "flow-var";
...
function renderFlowTemplateVarFields(detail = null, draft = {}) {
  if (detail !== undefined) {
    setLastFlowRunTemplateDetail(detail);
  }
  ...
  if (!schema.length) {
    container.innerHTML = "";
    hint.textContent = getLastFlowRunTemplateDetail()
...
function renderFlowTemplateVarFieldsError(message) {
  setLastFlowRunTemplateDetail(null);
  const container = byId("flow-vars-fields");
  const hint = byId("flow-vars-fields-hint");
  if (!container || !hint) return;
...
export function installRenderCommonRuntime() {
  Object.assign(window, {
    renderModeValidationCard,
    renderStatusMessageCard,
    renderStatusToast,
    renderFlowTemplateVarField,
