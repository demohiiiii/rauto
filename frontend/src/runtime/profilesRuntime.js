...
function renderDiagnoseProfileOptions(keyword = "") {
  populateSelectOptions(
    "profile-diagnose-picker",
    cachedDeviceProfiles.filter((name) => name !== "autodetect"),
    {
...
function setBuiltinForm(profile) {
  ...
  renderReadonlyDetectProfile(
    "builtin-detect-profile-list",
...
function renderReadonlyDetectProfile(containerId, detectProfile) {
  ...
  const probesContainer = probesWrap.querySelector(
    ".js-readonly-detect-probes",
  );
  const probes = Array.isArray(detectProfile.probes)
    ? detectProfile.probes
    : [];
  if (probes.length === 0) {
    const empty = document.createElement("div");
    empty.className = "text-xs text-slate-500";
  ...
  probes.forEach((probe) => {
    ...
    renderReadonlyDetectRules(
      row.querySelector(".js-probe-rules"),
      probe.rules,
    );
    const errors = Array.isArray(probe.error_patterns)
...
function addDetectProbeRow(
  ...
  (Array.isArray(item.error_patterns) ? item.error_patterns : []).forEach(
    (pattern) =>
      addPatternRow(row.querySelector(".js-detect-error-patterns"), pattern),
  );
  container.appendChild(row);
}
...
function collectDetectProfileForm() {
  ...
  const probes = Array.from(byId("detect-probes-list").children)
    .map((row) => {
      ...
      const rules = collectDetectRuleRows(
        row.querySelector(".js-detect-probe-rules"),
      );
      const errorPatterns = collectPatternRows(
        row.querySelector(".js-detect-error-patterns"),
      );
      if (!command && rules.length === 0 && errorPatterns.length === 0)
        return null;
      if (!command) {
...
function normalizeHookFlow(operation) {
  ...
  return {
    ...
    stop_on_error:
      flow.stop_on_error === undefined ? true : !!flow.stop_on_error,
...
function setHookOperationPanels(row) {
  ...
  row.querySelector(".js-hook-unsupported-panel").hidden =
    kind !== "unsupported";
}
...
function setHookOperationFields(row, operation) {
  ...
  if (operation.kind === "flow") {
    ...
    row.querySelector(".js-hook-flow-stop-on-error").checked =
      flow.stop_on_error;
    row.querySelector(".js-hook-flow-max-steps").value =
      flow.max_steps == null ? "" : String(flow.max_steps);
    flow.steps.forEach((step) =>
      addHookFlowStep(row.querySelector(".js-hook-flow-steps"), step),
    );
    setHookOperationPanels(row);
    return;
  }
  ...
  row.querySelector(".js-hook-unsupported-operation").value = JSON.stringify(
    operation,
    null,
    2,
  );
...
function collectHookOperation(row, triggerName, name) {
  ...
  if (kind === "flow") {
    const steps = Array.from(
      row.querySelector(".js-hook-flow-steps").children,
    ).map((step, idx) =>
    ...
    const maxStepsRaw = row
      .querySelector(".js-hook-flow-max-steps")
      .value.trim();
    if (maxStepsRaw) {
...
function addHookRow(containerId, item = {}, state = "") {
  ...
  row.querySelector(".js-add-hook-flow-step").onclick = () => {
    addHookFlowStep(
      row.querySelector(".js-hook-flow-steps"),
...
function collectStateHookRows(containerId, triggerName) {
  ...
  Array.from(byId(containerId).children).forEach((row) => {
    ...
    const operation = collectHookOperation(
      row,
      `${triggerName}/${state}`,
      name,
    );
...
function addReadonlyHookRow(container, trigger, item = {}, state = "") {
  ...
  if (operation.kind === "flow") {
    ...
    flow.steps.forEach((step, idx) => {
      ...
      stepRow.querySelector(".js-hook-command").value =
...
function addPromptRow(item = { state: "", patterns: [] }) {
  ...
  row
    .querySelector(".js-state")
    .addEventListener("input", updateHookModeOptions);
...
function setProfileForm(profile) {
  ...
  hooks.after_connect.forEach((item) =>
    addHookRow("hooks-after-connect-list", item),
  );
...
function collectProfileForm() {
  ...
  const profile = {
    ...
    hooks: {
      after_connect: collectHookRows(
        "hooks-after-connect-list",
        "after_connect",
      ),
      before_disconnect: collectHookRows(
        "hooks-before-disconnect-list",
        "before_disconnect",
      ),
...
export function installProfilesRuntime() {
  Object.assign(window, {
    ...
    updateBuiltinCommandExecutionVisibility,
    setBuiltinForm,
    addReadonlyDetectRuleRow,
    renderReadonlyDetectRules,
    renderReadonlyDetectProfile,
    addSimpleListRow,
    addReadonlySimpleListRow,
    collectSimpleList,
    clearContainer,
    updateProfileDetectVisibility,
    addDetectRuleRow,
    collectDetectRuleRows,
    addDetectProbeRow,
    ...
    normalizeHooks,
    profilePromptModes,
    applyHookModeOptions,
    updateHookModeOptions,
    normalizeHookCommand,
    normalizeHookFlow,
    hookOperationLabel,
    hookOperationKindLabel,
    setHookOperationPanels,
    addHookFlowStep,
    collectHookCommandFields,
    setHookOperationFields,
    hookRowHasOperationInput,
    collectHookOperation,
    addHookRow,
    collectHookRows,
    ...
    renderReadonlyHooks,
    addPromptRow,
    addReadonlyPromptRow,
    collectPromptRows,
    addSysPromptRow,
    addReadonlySysPromptRow,
    collectSysPromptRows,
    addInteractionRow,
    addReadonlyInteractionRow,
    collectInteractionRows,
    addPatternRow,
    addReadonlyPatternRow,
    collectPatternRows,
    addTransitionRow,
    addReadonlyTransitionRow,
