...
function derivedDisplayFieldStores(sourceStore, fieldMap, fallback = {}) {
  return Object.fromEntries(
    Object.entries(fieldMap).map(([storeName, fieldName]) => [
      storeName,
      deriveStore(
        sourceStore,
        (display) => display?.[fieldName] || fallback[fieldName] || {},
      ),
    ]),
  );
}
...
export function createTxWorkflowBlockResultPanelWorkspace(inputState = {}) {
  const workflowBlockRowStateStore = writable(
  ...
  return {
    ...
    setWorkflowBlockRow(workflowBlockRow = {}) {
      workflowBlockRowStateStore.set(
