...
function filteredWorkspaceItemsStore(stateStore, collectionKey, fields) {
  return derived(stateStore, (state) => {
    const query = trimmedText(state.search).toLowerCase();
    if (!query) return state[collectionKey];
    return state[collectionKey].filter((item) =>
      fields
...
async function runWorkspaceAction(stateStore, action, operation) {
