<script lang="ts">
  ...
  let {
    ...
    display: HookInteractionDisplay;
    idPrefix?: string;
    interaction?: HookInteraction;
    onChange?: (interaction: HookInteraction) => void;
  } = $props();
  ...
  function updatePrompts(prompts: HookPromptRule[]) {
    onChange?.({ prompts });
  }
  ...
  function patchPrompt(promptIndex: number, patch: Partial<HookPromptRule>) {
    updatePrompts(
  ...
  function addPattern(promptIndex: number) {
  ...
  function removePattern(promptIndex: number, patternIndex: number) {
    const prompt = currentPrompts()[promptIndex] || {};
    patchPrompt(promptIndex, {
