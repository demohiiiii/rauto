<script lang="ts">
  ...
  import { createBuiltinProfileStateListsSectionWorkspace } from "$domains/profiles/application/profilePanelWorkspaces.js";
  import type { BuiltinProfileDetail } from "../componentTypes.js";

  type PatternRow =
    BuiltinProfileDetail["promptRows"][number]["patternRows"][number];
...
{#snippet readonlySectionHeader(titleText: string)}
  <span class="min-w-0 break-words text-xs font-semibold sm:text-sm">
    {titleText}
  </span>
{/snippet}

{#snippet readonlyPatternRows(patternRows: PatternRow[])}
