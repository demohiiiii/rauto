<script lang="ts">
  ...
  import type {
    BuiltinHookCommandDisplay,
    BuiltinHookRowDisplay,
    BuiltinHookStepDisplay,
    BuiltinProfileDetail,
    HookInteractionDisplay,
...
{#snippet readonlyInput(labelText: string, value: string, inputClass = "")}
...
{#snippet readonlyTextArea(labelText: string, value: string)}
  <div class="grid min-w-0 gap-1.5">
    <span class="text-xs font-medium text-foreground">{labelText}</span>
...
{#snippet readonlySwitch(labelText: string, checked: boolean)}
  <div
...
{#snippet readonlyHookInteraction(interactionDisplay: HookInteractionDisplay)}
  <section
...
{#snippet readonlyHookStep(hookStep: BuiltinHookStepDisplay)}
...
{#snippet readonlyHookCommand(operation: BuiltinHookCommandDisplay)}
  <div
...
{#snippet hookReadonlyRow(hookRow: BuiltinHookRowDisplay)}
  <article
