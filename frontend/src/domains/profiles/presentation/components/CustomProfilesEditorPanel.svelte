<script lang="ts">
  ...
  import { customProfileListSectionDisplays } from "$domains/profiles/application/profileListState.js";
  import type { ProfileStatusTone } from "../componentTypes.js";
  ...
  type ProfileListSectionDisplay = ReturnType<
    typeof customProfileListSectionDisplays
  >[number];

  interface CustomProfilesEditorPanelProps {
    active: boolean;
    customStatusMessage: string;
    customStatusTone: ProfileStatusTone;
    customTitle: string;
    hooksHint: string;
    hooksTitle: string;
    onProfileIdentityChange?: (profileName: string) => void;
  ...
  let {
    ...
    showCustomStatus,
  }: CustomProfilesEditorPanelProps = $props();
  ...
  let lastReportedProfileName = $state<string | null>(null);
...
{#snippet profileListSectionEditor(
  profileListSection: ProfileListSectionDisplay,
)}
  <CollapsibleGroup
    variant="section"
    class="px-4 py-4"
