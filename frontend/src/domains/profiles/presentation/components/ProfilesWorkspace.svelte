<script lang="ts">
  ...
  import BuiltinProfileDetailsPanel from "./BuiltinProfileDetailsPanel.svelte";
  import CustomProfilesEditorPanel from "./CustomProfilesEditorPanel.svelte";
  import ProfileDiagnosePanel from "./ProfileDiagnosePanel.svelte";
  ...
  let selectedProfileKind = $state<ProfileKind | "">("");
  ...
  let profileRows = $derived.by<ProfileRow[]>(() => {
    currentLanguage;
  ...
  function setDiagnoseDialogOpen(open: boolean) {
    diagnoseDialogOpen = open;
  }

  async function selectProfile(profileRow: ProfileRow) {
