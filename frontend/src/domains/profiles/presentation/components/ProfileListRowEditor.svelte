<script lang="ts">
  ...
  type ProfileRowsByList =
    typeof profileListRowsState extends Readable<infer T> ? T : never;
  type ProfileListRow = ProfileRowsByList[string][number];
  type PatternValue = Parameters<
    NonNullable<ProfileListRowHandlerOptions["onPatternChange"]>
  >[2];
  type PatternState = Parameters<
    NonNullable<ProfileListRowHandlerOptions["onPatternStateChange"]>
  >[1];
  type ProfileListRowPatch = Parameters<
    NonNullable<ProfileListRowHandlerOptions["onProfileListRowChange"]>
  >[1];
  type SimpleValue = Parameters<
    NonNullable<ProfileListRowHandlerOptions["onSimpleValueChange"]>
  >[1];
  type ButtonVariant =
    | "default"
    | "destructive"
  ...
  interface ProfileListRowProps extends Omit<
  ...
  let {
    ...
    profileListRow,
    rowIndex,
  }: ProfileListRowProps = $props();

  function handleAddPattern(currentRowIndex: number) {
  ...
  function handlePatternStateChange(
    currentRowIndex: number,
  ...
  function handleProfileListRowChange(
    currentRowIndex: number,
  ...
  function handleRemovePattern(currentRowIndex: number, patternIndex: number) {
  ...
  function handleRemoveSimpleValue(currentRowIndex: number) {
    return typeof onRemoveSimpleValue === "function"
      ? onRemoveSimpleValue(currentRowIndex)
      : undefined;
  }

  function handleSimpleValueChange(
    currentRowIndex: number,
...
{#snippet patternList(
  patternRows: ProfilePatternRow[],
  patternLabel = labels.pattern,
)}
