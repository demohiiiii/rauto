<script lang="ts">
  ...
  import { customProfileHookSectionDisplays } from "$domains/profiles/application/profileListState.js";
  import type { ProfileHookRowHandlerOptions } from "$domains/profiles/application/profileEditorBindings.js";
  import type { HookFlowStepDisplay } from "../componentTypes.js";
  import ProfileHookInteractionEditor from "./ProfileHookInteractionEditor.svelte";

  type HookRow = ReturnType<
    typeof customProfileHookSectionDisplays
  >[number]["hookRows"][number];
  type HookRowPatch = Parameters<
    NonNullable<ProfileHookRowHandlerOptions["onHookRowChange"]>
  >[1];
  type HookCommandPatch = Parameters<
    NonNullable<ProfileHookRowHandlerOptions["onCommandChange"]>
  >[1];
  type HookFlowPatch = Parameters<
    NonNullable<ProfileHookRowHandlerOptions["onFlowChange"]>
  >[1];
  type HookFlowStepPatch = Parameters<
    NonNullable<ProfileHookRowHandlerOptions["onFlowStepChange"]>
  >[2];
  type HookKind = Parameters<
    NonNullable<ProfileHookRowHandlerOptions["onKindChange"]>
  >[1];

  interface ProfileHookRowProps extends Omit<
  ...
  let {
    ...
    rowIndex,
    stateList,
  }: ProfileHookRowProps = $props();

  function handleAddFlowStep(currentRowIndex: number) {
    return onAddFlowStep?.(currentRowIndex);
  }

  function handleCommandChange(
    currentRowIndex: number,
    patch: HookCommandPatch,
  ) {
    return onCommandChange?.(currentRowIndex, patch);
  }

  function handleFlowChange(currentRowIndex: number, patch: HookFlowPatch) {
    return onFlowChange?.(currentRowIndex, patch);
  }

  function handleFlowStepChange(
    currentRowIndex: number,
    stepIndex: number,
    patch: HookFlowStepPatch,
  ) {
    return onFlowStepChange?.(currentRowIndex, stepIndex, patch);
  }

  function handleKindChange(currentRowIndex: number, hookKind: HookKind) {
    return onKindChange?.(currentRowIndex, hookKind);
  }

  function handleRemoveFlowStep(currentRowIndex: number, stepIndex: number) {
    return onRemoveFlowStep?.(currentRowIndex, stepIndex);
  }

  function handleRemoveRow(currentRowIndex: number) {
    return onRemoveRow?.(currentRowIndex);
  }

  function handleHookRowChange(currentRowIndex: number, patch: HookRowPatch) {
  ...
  function changeOperationKind(nextKind: string) {
...
{#snippet fieldLabel(labelText: string)}
  <span class="text-xs font-medium text-foreground">{labelText}</span>
{/snippet}

{#snippet hookFlowStep(hookFlowStepRow: HookFlowStepDisplay)}
