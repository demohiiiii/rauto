<script lang="ts">
  ...
  type DetectProfile = BuiltinProfileDetail["detectProfile"];
  type DetectRuleRow = DetectProfile["initialRuleRows"][number];
  type DetectProbeRow = DetectProfile["probeRows"][number];
...
{#snippet readonlyFieldCard(readonlyValue: string)}
  <div class="rounded-lg border border-border bg-muted/40 p-3">
    <ReadonlyInputField value={readonlyValue} />
  </div>
{/snippet}

{#snippet emptyReadonlyText(message: string)}
  <div
...
{#snippet readonlyPatternList(
  patternValues: string[],
  hasPatternValues: boolean,
  emptyMessage: string,
)}
...
{#snippet detectRuleRows(
  detectRuleRowValues: DetectRuleRow[],
  hasDetectRuleRows: boolean,
)}
  <div class="grid gap-2">
...
{#snippet detectProbeCard(detectProbeRow: DetectProbeRow)}
