<script lang="ts">
  ...
  type DetailModalWorkspace = ReturnType<
    (typeof import("$domains/overlays/index.js"))["createDetailModalWorkspace"]
  >;
  type ContentDisplay = StoreValue<
    DetailModalWorkspace["detailModalContentPropsStateStore"]
  >["contentDisplay"];
  type HistoryEventRow =
    ContentDisplay["historyDetailDisplay"]["entryRows"][number];

  let { contentDisplay }: { contentDisplay: ContentDisplay } = $props();
  let OrchestrationDetailComponent = $derived(
    contentDisplay.OrchestrationDetailComponent as Component | null,
  );
  const metaIconRows = [
    ClockIcon,
    CircleDotIcon,
  ...
  function metaIcon(metaIndex: number) {
    return metaIconRows[metaIndex] || CircleDotIcon;
  }

  function statIcon(statIndex: number) {
    return statIconRows[statIndex] || ListChecksIcon;
  }

  function statTone(statIndex: number): string {
    return statToneRows[statIndex] || "bg-secondary text-secondary-foreground";
  }

  function openHistoryEvent(entryIndex: number): void {
...
{#snippet historyEventCard(historyEventRow: HistoryEventRow)}
