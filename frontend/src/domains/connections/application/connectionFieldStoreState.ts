...
import type {
  ...
  ConnectionPickerKind,
  ConnectionPickerState,
  ConnectionVarRow,
  ConnectionVarsState,
  InventoryNamedResource,
  SavedConnection,
  ShowObjectInput,
  ShowObjectOption,
} from "../model/types.js";
...
const CONNECTION_VARS_ROW_KEY_PREFIX: Readonly<Record<string, string>> =
  Object.freeze({
...
const pickerConfigs = (
  ...
  keys: string[],
  options: Omit<ConnectionPickerConfig, "kind"> = {},
): Record<string, ConnectionPickerConfig> =>
  Object.fromEntries(keys.map((key) => [key, { kind, ...options }]));
const CONNECTION_PICKER_CONFIGS: Record<string, ConnectionPickerConfig> = {
...
const connectionPickerValueState = new Map<string, string[]>();
const connectionPickerStates = new Map<
  string,
  Writable<ConnectionPickerState>
>();
const connectionProfileSelectStates = new Map<
  string,
  Writable<{ profiles: string[] }>
>();
const connectionVarsValueState = new Map<string, Record<string, unknown>>();
const connectionVarsStates = new Map<string, Writable<ConnectionVarsState>>();
const showObjectOptionsState = new Map<string, ShowObjectOption[]>();
...
function normalizeMappedKey(
  key: unknown,
  validKeys: ReadonlySet<string> | null = null,
  fallback = "",
): string {
...
function normalizeConnectionPickerKey(key: unknown): string {
  return normalizeMappedKey(key, CONNECTION_PICKER_KEYS);
}

function normalizeConnectionVarsKey(key: unknown): string {
  return normalizeMappedKey(key, CONNECTION_VARS_KEYS);
}

function normalizeConnectionProfileSelectKey(key: unknown): string {
...
function connectionPickerConfig(key: unknown): ConnectionPickerConfig | null {
...
function connectionProfileSelectStoreFor(key: unknown) {
  const normalizedKey = normalizeConnectionProfileSelectKey(key);
  const existingStore = connectionProfileSelectStates.get(normalizedKey);
  if (existingStore) return existingStore;
  const profileStore = writable(connectionProfileSelectPayload());
  connectionProfileSelectStates.set(normalizedKey, profileStore);
  return profileStore;
}

function updateConnectionProfileSelectState(key: unknown): void {
  const normalizedKey = normalizeConnectionProfileSelectKey(key);
...
function normalizeConnectionPickerValues(pickerValues: unknown[] = []) {
...
function normalizeShowObjectOptions(
  objects: ShowObjectInput[] = [],
): ShowObjectOption[] {
...
function defaultConnectionPickerState(key: unknown): ConnectionPickerState {
...
function connectionPickerStoreFor(
  key: unknown,
): Writable<ConnectionPickerState> {
  const pickerKey = normalizeConnectionPickerKey(key);
...
function updateConnectionPickerState(
  key: unknown,
  updater: (state: ConnectionPickerState) => ConnectionPickerState,
): void {
  const pickerKey = normalizeConnectionPickerKey(key);
...
function showObjectOptionValues(key: unknown): string[] {
...
export function showObjectOptionMeta(
  pickerKey: unknown,
  showObjectValue: unknown = "",
): ShowObjectOption {
...
function connectionPickerOptionRow(
  kind: ConnectionPickerKind | "",
  pickerKey: unknown,
  optionValue: string,
) {
...
function refreshConnectionPickerMenu(key: unknown): void {
  updateConnectionPickerState(key, (state) => ({ ...state }));
}

function refreshConnectionPicker(key: unknown): void {
  refreshConnectionPickerSelected(key);
  refreshConnectionPickerMenu(key);
}

function refreshConnectionGroupsForPicker(
  pickerKey: unknown,
  selectedValues: unknown[] | null = null,
): void {
...
function connectionVarValueToInput(varValue: unknown): string {
...
function parseConnectionVarValue(type: string, rawValue: unknown): unknown {
...
function normalizeConnectionVarRows(
  key: unknown,
...
function connectionVarRowsToConnectionVars(
  ...
  const connectionVars: Record<string, unknown> = {};
...
function setConnectionVarsState(
  key: unknown,
...
function defaultConnectionVarsState(key: unknown): ConnectionVarsState {
  const varsKey = normalizeConnectionVarsKey(key);
...
function connectionVarsStoreFor(key: unknown): Writable<ConnectionVarsState> {
  const varsKey = normalizeConnectionVarsKey(key);
...
function publishConnectionVarsState(
  key: unknown,
  connectionVarRows: ConnectionVarRow[],
  connectionVars: Record<string, unknown>,
): void {
...
function applyConnectionVarsRowPatch(
  key: unknown,
  id: string | number | undefined,
  patch: Partial<ConnectionVarRow> = {},
): void {
  const varsKey = normalizeConnectionVarsKey(key);
...
export function connectionProfileSelectState(key: unknown) {
  return connectionProfileSelectStoreFor(key);
}

export function setConnectionDeviceProfiles(profiles: unknown[] = []) {
...
export function connectionPickerState(key: unknown) {
  return connectionPickerStoreFor(key);
}

export function connectionPickerValues(key: unknown): string[] {
...
export function connectionPickerCommitKeys(key: unknown): string[] {
  const pickerConfig = connectionPickerConfig(key);
  return pickerConfig?.kind === "labels" ? ["Enter", ","] : ["Enter"];
}

function setConnectionPickerOpen(key: unknown, open = true): void {
  updateConnectionPickerState(key, (state) => ({ ...state, open: !!open }));
}

function setConnectionPickerQuery(key: unknown, query: unknown = ""): void {
...
export function hideConnectionPickerMenu(key: unknown): void {
  setConnectionPickerOpen(key, false);
}

export function openConnectionPickerMenu(key: unknown): void {
  setConnectionPickerOpen(key, true);
}

export function setConnectionPickerQueryValue(
  key: unknown,
  query: unknown = "",
): void {
...
function getConnectionPickerSelectedValues(key: unknown): string[] {
  return connectionPickerValues(key);
}

export function refreshConnectionPickerSelected(key: unknown): void {
  updateConnectionPickerState(key, (state) => ({ ...state }));
}

export function setShowObjectPickerOptions(
  key: unknown,
  options: ShowObjectInput[] = [],
  selectedValues: unknown[] | null = null,
  onSelectionChange: ((values: string[]) => unknown) | null = null,
) {
  const pickerKey = normalizeConnectionPickerKey(key);
...
function addConnectionPickerValue(key: unknown, rawValue: unknown) {
...
function removeConnectionPickerValue(key: unknown, rawValue: unknown) {
  const nextValue = safeString(rawValue || "").trim();
  if (!nextValue) return false;
  const pickerKey = normalizeConnectionPickerKey(key);
...
function commitConnectionPickerInput(key: unknown, rawInput: unknown = "") {
...
export function addConnectionPickerSelection(key: unknown, rawValue: unknown) {
  return addConnectionPickerValue(key, rawValue);
}

export function removeConnectionPickerSelection(
  key: unknown,
  rawValue: unknown,
) {
...
export function commitConnectionPickerSelection(
  key: unknown,
  rawInput: unknown = "",
) {
...
function refreshSavedPickerOptions(
  kind: "groups" | "labels",
  selectedValues: unknown[] | null = null,
): void {
  const pickerKeys =
...
export function setConnectionPickerSavedConnections(
...
export function setConnectionPickerSelectedValues(
  key: unknown,
...
export function refreshConnectionPickerOptions(key: unknown) {
...
export function getConnectionLabelValues(key: unknown) {
  return connectionPickerValues(key);
}

export function getConnectionGroupValues(key: unknown) {
  return connectionPickerValues(key);
}

export function setConnectionVarsValue(
  key: unknown,
  vars: Record<string, unknown> = {},
) {
  const varsKey = normalizeConnectionVarsKey(key);
...
export function getConnectionVarsValue(key: unknown) {
  const varsKey = normalizeConnectionVarsKey(key);
  const connectionVarRows =
    getStore(connectionVarsStoreFor(varsKey)).connectionVarRows || [];
...
export const connectionVarsState = (key: unknown) =>
  connectionVarsStoreFor(key);

function connectionVarRowPresentation(connectionVarRow: ConnectionVarRow = {}) {
...
export function connectionVarRowsForState(
  connectionVarsState: ConnectionVarsState = {},
) {
...
export function addConnectionVarsRow(key: unknown) {
  const varsKey = normalizeConnectionVarsKey(key);
  const connectionVarRows =
    getStore(connectionVarsStoreFor(varsKey)).connectionVarRows || [];
...
export function removeConnectionVarsRow(
  key: unknown,
  id: string | number | undefined,
) {
  const varsKey = normalizeConnectionVarsKey(key);
...
export const setConnectionVarRowName = (
  key: unknown,
  id: string | number | undefined,
  connectionVarName: unknown = "",
) =>
  applyConnectionVarsRowPatch(key, id, {
    name: displayString(connectionVarName || ""),
  });

export const setConnectionVarRowType = (
  key: unknown,
  id: string | number | undefined,
  connectionVarType: unknown = "",
) =>
  applyConnectionVarsRowPatch(key, id, {
    type: displayString(connectionVarType || ""),
  });

export const setConnectionVarRowValue = (
  key: unknown,
  id: string | number | undefined,
  connectionVarValue: unknown = "",
) =>
  applyConnectionVarsRowPatch(key, id, {
