...
import { openSavedConnectionEditor } from "./connectionEditorState.js";
...
export function createSavedConnectionLibraryWorkspace() {
  ...
  const savedConnectionLoadingState: { keys: string[] } = { keys: [] };
  ...
  function setPanelContext({ active = false }: { active?: boolean } = {}) {
    activeStateStore.set(!!active);
  }

  function selectSavedConnection(selectedName: string) {
  ...
  async function importConnections(file: File | null) {
    return importConnectionsFromFile(file);
  }

  async function useSavedConnection(onUse: (() => unknown) | null) {
