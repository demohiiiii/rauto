<script lang="ts">
  ...
  let { model = defaultInteractiveTemplateModel() }: Props = $props();
  let currentLanguage = $derived($currentLanguageState);
  let display = $derived.by(() => {
    currentLanguage;
    return interactiveReadonlyPresentation(model);
  });
</script>
  ...
  >
    <div class="flex min-w-0 flex-wrap items-center justify-between gap-2">
      <span class="text-sm font-semibold text-foreground">
        {display.command.titleText}
      </span>
      <div class="flex min-w-0 flex-wrap items-center gap-2">
        <Badge variant="outline">
          {display.command.modeLabelText}: {display.command.modeText}
        </Badge>
        <Badge variant="secondary">
          {display.command.timeoutLabelText}: {display.command.timeoutText}
        </Badge>
        <Badge variant="outline">
          {display.command.multilineModeLabelText}: {display.command
            .multilineModeText}
        </Badge>
      </div>
    </div>
    ...
    <div class="grid min-w-0 gap-1 rounded-lg bg-muted/30 p-3">
      <span class="text-xs font-medium text-muted-foreground">
        {display.command.commandLabelText}
      </span>
      <code class="break-all whitespace-pre-wrap text-sm text-foreground">
        {display.command.commandText || "-"}
      </code>
    </div>

    {#if display.command.promptRows.length > 0}
      <div class="grid min-w-0 gap-2 border-t border-border pt-3">
        {#each display.command.promptRows as promptRow}
