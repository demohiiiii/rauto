<script lang="ts">
  ...
  import type { TemplateManagerSection } from "../../model/types.js";

  type ConfigCommand = Awaited<ReturnType<typeof listConfigCommands>>[number];
  type ConfigVolatilePattern = Awaited<
    ReturnType<typeof listConfigVolatilePatterns>
  >[number];

  let { definition }: { definition: TemplateManagerSection } = $props();
  ...
  let commands = $state<ConfigCommand[]>([]);
  let patterns = $state<ConfigVolatilePattern[]>([]);
  let profiles = $state<string[]>([]);
  let profileModes = $state<string[]>([]);
  ...
  function modeValueMatchesProfileModes(modeValue: string): boolean {
    return profileModeExpressionMatchesOptions(modeValue, profileModes);
  }

  async function loadCommandProfileModes(profile: string): Promise<void> {
    ...
    try {
      ...
      if (requestSequence !== modeRequestSequence) return;
      showToast(`${labels.requestFailed}: ${errorMessage(error)}`, "error");
    } finally {
      if (requestSequence === modeRequestSequence) loadingModes = false;
    }
  }

  function selectCommandProfile(profile: string): void {
  ...
  async function saveCommandOverride(): Promise<void> {
    savingCommand = true;
    try {
  ...
  async function removeCommandOverride(row: ConfigCommand): Promise<void> {
  ...
  async function removePattern(row: ConfigVolatilePattern): Promise<void> {
    if (!browserConfirm(labels.deleteConfirm)) return;
    try {
