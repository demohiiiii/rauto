<script lang="ts">
  ...
  import type { TemplateManagerKind } from "../../model/types.js";
  import { createContentTemplateWorkspace } from "../../application/createContentTemplateWorkspace.js";
  import { createShowObjectWorkspace } from "../../application/createShowObjectWorkspace.js";
  import { createTextfsmMappingWorkspace } from "../../application/createTextfsmMappingWorkspace.js";
  import ConfigCatalogWorkspace from "./ConfigCatalogWorkspace.svelte";
  import ShowObjectWorkspace from "./ShowObjectWorkspace.svelte";
  import TemplateCatalogPanel from "./TemplateCatalogPanel.svelte";
  import TextfsmMappingWorkspace from "./TextfsmMappingWorkspace.svelte";
  ...
  let activeSectionKey = $state<TemplateManagerKind>(
  ...
  async function selectSection(sectionKey: string): Promise<void> {
    const section = templateManagerSections.find(
      (candidate) => candidate.key === sectionKey,
    );
    if (!section || section.key === activeSectionKey) return;
    const nextSectionKey = section.key;
    if (contentTemplateKinds.has(nextSectionKey)) {
      const activated = await contentWorkspace.activate(nextSectionKey);
      if (!activated) return;
    }
    activeSectionKey = nextSectionKey;
    if (
      nextSectionKey === TEMPLATE_MANAGER_KIND.textfsmMappings &&
      !mappingLoaded
    ) {
      mappingLoaded = true;
    ...
    if (
      nextSectionKey === TEMPLATE_MANAGER_KIND.showObjects &&
      !showObjectsLoaded
    ) {
