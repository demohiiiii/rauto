<script lang="ts">
  ...
  type NameDialogMode = "new" | "saveAs";
  ...
  let {
    ...
    definition: TemplateManagerSection;
  ...
  let nameDialog: NameDialogState = $state({
    open: false,
    mode: "new",
    value: "",
    error: "",
  ...
  let format = $derived(
    templateResourceDefinitions[templateState.kind]?.format || "text",
  );
  let selected = $derived(templateState.selected);
  let canSave = $derived(
    !!selected &&
      !selected.builtin &&
  ...
  let lineCount = $derived(
    templateState.content ? templateState.content.split("\n").length : 0,
  );
  let byteCount = $derived(
    new TextEncoder().encode(templateState.content || "").length,
  );
  ...
  function setNameDialogOpen(open: boolean): void {
...
<div class="flex min-w-0 flex-col gap-4">
  ...
  >
    <div class="min-w-0">
      <div class="flex flex-wrap items-center gap-2">
        ...
        <Badge variant="secondary">{templateState.items.length}</Badge>
    ...
    <div class="flex shrink-0 flex-wrap items-center gap-2">
      <LoadingButton
        variant="outline"
        size="sm"
        loading={templateState.loadingAction === "list"}
        disabled={busy}
  ...
  {#if templateState.errorMessage}
    <Alert.Root variant="destructive">
      ...
      <Alert.Description>{templateState.errorMessage}</Alert.Description>
    </Alert.Root>
  {/if}
  ...
  <div class="grid min-w-0 gap-4 xl:grid-cols-[17rem_minmax(0,1fr)]">
    <Card.Root class="min-w-0 self-start overflow-hidden shadow-xs">
      ...
      <Card.Content class="flex flex-col gap-1 p-1.5">
        {#if templateState.loadingAction === "list" && !templateState.loaded}
          {#each Array(4) as _}
    ...
    <Card.Root class="min-w-0 overflow-hidden shadow-xs">
      ...
      {:else}
        <Card.Header class="border-b bg-muted/15">
          <div class="min-w-0">
            <div class="flex flex-wrap items-center gap-2">
              ...
              <Badge variant={templateState.dirty ? "destructive" : "outline"}>
                {templateState.dirty ? labels.unsaved : labels.saved}
              </Badge>
            </div>
          ...
          <Card.Action class="flex flex-wrap justify-end gap-2">
            ...
            {#if !selected.builtin}
              <LoadingButton
                size="sm"
                loading={templateState.loadingAction === "save" ||
                  templateState.loadingAction === "create"}
        ...
        <Card.Content class="flex min-w-0 flex-col gap-4 p-4">
          {#if templateState.varsSchema.length}
            <div class="rounded-lg border bg-muted/20 p-3">
              ...
              <div class="flex flex-wrap gap-2">
                {#each templateState.varsSchema as field (field.name)}
        ...
        {#if !selected.builtin && !selected.isDraft}
          <Card.Footer class="justify-between border-t bg-muted/10">
            ...
            <LoadingButton
              variant="destructive"
              size="sm"
              loading={templateState.loadingAction === "delete"}
              disabled={busy}
