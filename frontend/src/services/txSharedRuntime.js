...
function txVarsAssistantConfig(textareaId) {
  return (
    TX_VARS_ASSISTANTS.find((item) => item.textareaId === textareaId) || null
  );
}

function txVarsAssistantEntry(key = "", type = "string", valueText = "") {
...
function txVarsAssistantRender(config) {
  ...
  form.innerHTML = entries
    .map((entry) => txVarsAssistantRowHtml(entry))
    .join("");
}
...
function txVarsAssistantSyncTextarea(config, { notify = false } = {}) {
  ...
  const next = JSON.stringify(
    txVarsAssistantEntriesToObject(state.entries),
    null,
    2,
  );
  ...
  if (raw) {
    try {
      ...
      if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
        throw new Error(tr("txVarsFormJsonObjectRequired"));
      }
    } catch (error) {
      if (!silent) {
        const message = error?.message || tr("requestFailed");
        setStatus(
          config.statusId,
          `${tr("txVarsFormJsonInvalid")}: ${message}`,
          "error",
        );
      }
      if (!keepStateOnError) {
...
function txVarsAssistantUpdateEntryFromRow(config, rowEl) {
  ...
  entry.key = safeString(keyInput?.value);
  entry.type = safeString(typeSelect?.value).trim() || "string";
  entry.valueText = safeString(valueInput?.value);
  return entry;
}
...
function bindTxVarsAssistant(config) {
  ...
  syncBtn.onclick = () => {
    if (txVarsAssistantSyncFromTextarea(config.textareaId)) {
      setStatus(config.statusId, tr("txVarsFormSynced"), "success");
    }
  };
  clearBtn.onclick = () => {
    const state = txVarsAssistantGetState(config);
...
export function installTxSharedRuntime() {
  ...
  window.defaultTxWorkflowTemplatePayload =
            ...
            steps: [
              ...
              },
              {
                run: {
                  ...
                  mode: "User",
                  command:
                    "echo version=2026.04.17 > /tmp/rauto-demo/release.txt",
                  timeout: 30,
                },
                rollback: {
                  kind: "command",
  ...
  window.defaultOrchestrationTemplatePayload =
  ...
  window.setupTxVarsAssistants = function setupTxVarsAssistants() {
    TX_VARS_ASSISTANTS.forEach((config) => bindTxVarsAssistant(config));
  };

  window.rerenderTxVarsAssistants = function rerenderTxVarsAssistants() {
