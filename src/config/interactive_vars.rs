...
impl InteractiveVarResolver {
    ...
    fn fill_template_inline_references(&mut self, template: &InteractiveTemplate) {
...
pub fn resolve_runtime_var_aliases(
    ...
    let mut resolver = InteractiveVarResolver::new(runtime_vars, current)?;
    resolver.resolve_runtime_aliases();
    Ok(Value::Object(resolver.runtime_vars))
}

pub fn resolve_interactive_runtime_vars(
...
pub fn interactive_runtime_var_names(template: &InteractiveTemplate) -> Vec<String> {
...
fn collect_template_inline_references(template: &InteractiveTemplate) -> Vec<String> {
...
mod tests {
    ...
    use crate::config::interactive_template::parse_interactive_template_str;
    use crate::db;
    use serde_json::json;
    ...
    fn sample_credential() -> anyhow::Result<String> {
        let credential = create_credential(&DeviceCredentialInput {
            name: format!("interactive-test-{}", rand::random::<u64>()),
...
"#,
        ...
        let resolved =
            resolve_interactive_runtime_vars(&template, Value::Null, Some(current)).expect("ok");
...
"#,
        ...
        let resolved = resolve_interactive_runtime_vars(
            &template,
...
"#,
        ...
        let resolved =
            resolve_interactive_runtime_vars(&template, json!({"service":"nginx"}), Some(current))
                .expect("resolve");
