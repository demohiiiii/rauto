...
#[cfg(test)]
pub struct TestDbPathGuard {
    previous: Option<PathBuf>,
    _not_send: std::marker::PhantomData<std::rc::Rc<()>>,
}

#[cfg(test)]
pub fn set_test_db_path_for_current_thread(path: PathBuf) {
    TEST_DB_PATH_OVERRIDE.with(|value| *value.borrow_mut() = Some(path));
}

#[cfg(test)]
pub fn override_test_db_path(path: PathBuf) -> TestDbPathGuard {
...
#[cfg(test)]
pub async fn close_test_db(path: &std::path::Path) {
...
fn default_db_path() -> PathBuf {
    if let Some(path) = std::env::var_os("RAUTO_HOME").filter(|value| !value.is_empty()) {
        return PathBuf::from(path).join("rauto.db");
    }
    dirs::home_dir()
        .map(|path| path.join(".rauto").join("rauto.db"))
        .unwrap_or_else(|| PathBuf::from(".rauto/rauto.db"))
}
