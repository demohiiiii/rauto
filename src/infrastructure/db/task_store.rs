pub use crate::domain::task::{TaskArtifactRecord, TaskEventRecord, TaskRunRecord};
use crate::domain::task::{TaskCallback, TaskEvent, TaskOperation};
...
pub fn save_task_callback(callback: &TaskCallback, operation: TaskOperation) -> Result<()> {
    ...
    db::run_sync(async move {
        ...
        let success = matches!(callback.status, crate::domain::task::TaskStatus::Success);
