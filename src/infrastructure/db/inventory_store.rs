pub use crate::domain::connection::InventoryGroup;
use crate::domain::connection::{
    normalize_name_list, normalize_optional_text, normalize_simple_name,
};
...
fn normalize_inventory_name(raw: &str) -> Result<String> {
    normalize_simple_name(raw).map_err(Into::into)
}

fn normalize_string_list(values: &[String]) -> Result<Vec<String>> {
    normalize_name_list(values).map_err(Into::into)
}
...
mod tests {
    ...
    use crate::infrastructure::db;
    use crate::infrastructure::db::connection_store::{SavedConnection, save_connection};
    use anyhow::Result;
    use serde_json::json;
    use std::fs;
