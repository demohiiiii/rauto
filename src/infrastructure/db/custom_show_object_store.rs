...
use crate::infrastructure::db::profile;
use anyhow::{Result, anyhow};
use sqlx::Row;
use std::time::{SystemTime, UNIX_EPOCH};

pub use crate::domain::device::CustomShowObject;
...
pub fn upsert(
    ...
    let mode = normalize_optional_text(mode)
        .map(|mode| profile::resolve_profile_mode(&profile, Some(&mode)))
        .transpose()?;
...
fn normalize_object(raw: &str) -> Result<String> {
    crate::domain::device::normalize_show_object(raw)
        .ok_or_else(|| anyhow!("show object must not be empty"))
}
