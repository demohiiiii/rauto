---
name: rauto-usage
description: "Install, upgrade, verify, and execute rauto for the user: bootstrap local rauto usage from official GitHub Releases binaries by default, then run device commands, template execution, tx block, tx workflow, replay, backup/restore, and connection/profile/template/history operations. Trigger on requests like install rauto, set up rauto on this machine, download rauto from GitHub Releases, upgrade/update rauto, check rauto version/path, run command for me, execute on device, check history, run workflow, diagnose profile, manage templates/profiles/connections, replay records, or restore backup."
---
...
4. For config-changing operations, prefer `tx`/`tx-workflow` with rollback.
...
- Prefer `tx`/`tx-workflow` with rollback over direct apply patterns.
