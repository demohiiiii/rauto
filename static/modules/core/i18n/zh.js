const i18nZh = {
  ...
  "connectionWorkspaceSubtitleTemporary": "在此表单可新建/修改已保存连接，也可应用为仅当前会话使用的临时目标。",
  ...
  "sidebarConnectionTemporaryPrompt": "当前是临时目标模式。填写主机、用户名、密码和 profile 后，可以仅为这次浏览器会话应用临时连接。",
  ...
  "savedConnEditHint": "修改当前连接配置后，可以执行 profile 探测，并决定是否替换已保存的 profile。",
  ...
  "savedConnAutodetectMissing": "请先执行 profile 探测，再决定是否替换已保存的 profile。",
  "close": "关闭",
  "cancel": "取消",
  "newBtn": "新增",
  "editingNew": "正在编辑新的配置",
  ...
  "agentAuthRequired": "当前是 Agent 托管模式，浏览器在调用受保护 API 前需要先提供 API token。",
  ...
  "txBlockDirectVarsPlaceholder": "事务块变量 JSON（可选），例如 {\"peer_host\":\"edge94.host\"}",
  "txBlockDirectVarsHint": "直接执行模式的运行时变量（可选），支持连接别名与当前连接参数回退查找。",
  ...
  "txBlockTemplateVarsPlaceholder": "事务块模板变量 JSON（可选），例如 {\"peer_host\":\"edge94.host\"}",
  "txStageBlock": "事务块",
  ...
  "txWorkflowEditorBridgeRunKindUnsupported": "当前工作流 Block 编辑仅支持命令模式事务块（commands）",
  ...
  "txWorkflowBlockTemplateVarsPlaceholder": "事务块模板变量 JSON（可选），例如 {\"peer\":\"edge94\"}",
  ...
  "txWorkflowVarsPlaceholder": "workflow 变量 JSON（可选），例如 {\"peer_host\":\"edge94.host\"}",
  "txWorkflowVarsHint": "支持直接跨连接引用（如 edge94.host）；仅写变量名时会回退到当前目标连接参数。",
  ...
  "txWorkflowLoadUnsupportedTemplateContent": "工作流构建器回填当前仅支持 tx_block_template_name 方式",
  "txWorkflowLoadUnsupportedOperation": "工作流构建器回填当前仅支持 command 类型的步骤与回滚操作",
  ...
  "orchestrationTemplateRunHint": "选择已保存编排模板，并结合运行时变量直接执行。",
  "orchestrationTemplateEditHint": "可在下方 JSON 编辑器中修改已加载的编排计划，再进行预览或执行。",
  "orchestrationDirectHint": "使用编排计划 JSON 进行直接执行。",
  "orchestrationEditorTitle": "编排 JSON",
  "orchestrationJsonHint": "编辑原始编排计划 JSON。模板执行也会复用这一编辑器进行预览与微调。",
  "orchestrationBaseDirPlaceholder": "inventory/workflow 相对路径的基准目录（可选）",
  "orchestrationVarsPlaceholder": "plan 变量 JSON（可选），例如 {\"peer_host\":\"edge94.host\"}",
  "orchestrationVarsHint": "模板字符串支持 {{ peer_host }}（变量+回退）或直接 {{ edge94.host }}。",
  ...
  "templateUseBtn": "打开",
  "flowHint": "可使用已保存或内置命令流程模板执行向导式 CLI 交互。变量可选，会并入模板渲染上下文。",
  "flowVarsFieldsTitle": "模板变量",
  "flowVarsFieldsHint": "模板声明的变量会在这里自动渲染。",
  "flowVarsFieldsEmpty": "该模板没有声明变量。",
  "flowVarsJsonHint": "JSON 区域可用于填写高级覆盖值。引用值支持 connection.param，也支持仅写 param 后自动回退查询。",
  "flowVarTypeLabel": "类型",
  ...
  "flowBuiltinTemplateHint": "可在这里查看内置模板转换后的 TOML，并复制到自定义编辑器继续修改保存。",
  ...
  "profileHooksHint": "声明式会话生命周期 Hook。mode 下拉选项来自当前 profile 已填写的 prompts 状态。",
  ...
  "textfsmPlatformPlaceholder": "TextFSM 平台覆盖，例如 cisco_ios",
  ...
  "flowTemplateManageHint": "上面的模板名称会在执行时生效。可以先加载已有命令流程模板编辑其 TOML，也可以把编辑器中的内容保存成可复用模板。",
